...
func TestPrivateShareAccessMatrix(t *testing.T) {
	...
	if _, err := teamSvc.AddMember(teamOwner, tm.ID, member, team.RoleEditor); err != nil {
		t.Fatal(err)
	}
	if _, err := teamSvc.AddMember(teamOwner, tm.ID, viewer, team.RoleViewer); err != nil {
		t.Fatal(err)
	}
...
func TestPrivateShareAccessInvalidatedByRemovalAndRevoke(t *testing.T) {
	...
	if _, err := teamSvc.AddMember(teamOwner, tm.ID, member, team.RoleViewer); err != nil {
		t.Fatal(err)
	}

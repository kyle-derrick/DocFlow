...
// TestResolveMatrix 覆盖求值矩阵：近覆盖远 / deny 优先 / user 覆盖 team /
// perm 不参与 / 无匹配回退（matched=false）。
func TestResolveMatrix(t *testing.T) {
	user, other, team := uuid.New(), uuid.New(), uuid.New()
	otherTeam := uuid.New()
	...
	tests := []struct {
		...
		{
			name:    "user deny beats team allow at same node",
			chain:   []ChainNode{{Entries: []Entry{entry(SubjectUser, user, EffectDeny, "read"), entry(SubjectTeam, team, EffectAllow, "read")}}},
			perm:    "read",
			allowed: false, matched: true,
		},
		{
			name:    "user allow beats team deny at same node",
			chain:   []ChainNode{{Entries: []Entry{entry(SubjectUser, user, EffectAllow, "read"), entry(SubjectTeam, team, EffectDeny, "read")}}},
			perm:    "read",
			allowed: true, matched: true,
		},
		{
			name:    "team deny at near node beats user allow at far node",
			chain:   []ChainNode{{Entries: []Entry{entry(SubjectTeam, team, EffectDeny, "delete")}}, {Entries: []Entry{entry(SubjectUser, user, EffectAllow, "delete")}}},
			perm:    "delete",
			allowed: false, matched: true,
		},
		{
			name:    "team allow grants member",
			chain:   []ChainNode{{Entries: []Entry{entry(SubjectTeam, team, EffectAllow, "share")}}},
			perm:    "share",
			allowed: true, matched: true,
		},
		{
			name:    "entries for other perm do not participate",
			chain:   []ChainNode{{Entries: []Entry{entry(SubjectTeam, team, EffectDeny, "write")}}},
			perm:    "read",
			allowed: false, matched: false,
		},
		{
			name:    "entries for other subject do not participate",
			chain:   []ChainNode{{Entries: []Entry{entry(SubjectUser, other, EffectDeny, "read"), entry(SubjectTeam, otherTeam, EffectDeny, "read")}}},
			perm:    "read",
			allowed: false, matched: false,
		},
		{
	...
	for _, tc := range tests {
		allowed, matched := Resolve(tc.chain, user, team, tc.perm)
...
// TestServiceReplaceValidation 覆盖 PUT 校验：个人空间目录拒绝、非法条目
// 拒绝、重复主体拒绝、合法整体替换（含清空）。
func TestServiceReplaceValidation(t *testing.T) {
	teamID := uuid.New()
	owner := uuid.New()
	personal := files.File{ID: uuid.New(), Name: "p", OwnerID: owner, Type: "folder", ScopeType: "personal"}
	teamFolder := files.File{ID: uuid.New(), Name: "t", OwnerID: owner, Type: "folder", ScopeType: "team", TeamID: &teamID}
	repo := NewMemoryRepo()
	repo.PutFolder(personal)
	repo.PutFolder(teamFolder)
	svc := NewService(repo)

	if _, err := svc.Replace(personal.ID, []EntryInput{{SubjectType: SubjectUser, SubjectID: owner, Effect: EffectAllow, Permissions: []string{"read"}}}, owner); !errors.Is(err, ErrNotTeamFolder) {
		t.Fatalf("personal folder: err = %v, want ErrNotTeamFolder", err)
	}
	if _, _, err := svc.List(personal.ID); !errors.Is(err, ErrNotTeamFolder) {
		t.Fatalf("list personal folder: err = %v, want ErrNotTeamFolder", err)
	}
	...
	for i, in := range bad {
		if _, err := svc.Replace(teamFolder.ID, []EntryInput{in}, owner); !errors.Is(err, ErrInvalidEntry) {
	...
	if _, err := svc.Replace(teamFolder.ID, dup, owner); !errors.Is(err, ErrDuplicateEntry) {
		t.Fatalf("duplicate subject: err = %v, want ErrDuplicateEntry", err)
	}

	valid := []EntryInput{
		{SubjectType: SubjectTeam, SubjectID: teamID, Effect: EffectDeny, Permissions: []string{"write", "delete"}},
		{SubjectType: SubjectUser, SubjectID: owner, Effect: EffectAllow, Permissions: []string{"read", "write", "delete", "share"}},
	}
	f, err := svc.Replace(teamFolder.ID, valid, owner)
	if err != nil {
		t.Fatalf("valid replace: %v", err)
	}
	if f.ID != teamFolder.ID {
		t.Fatalf("returned folder = %v, want %v", f.ID, teamFolder.ID)
	}
	_, entries, err := svc.List(teamFolder.ID)
	if err != nil {
	...
	// 清空：空数组整体替换。
	if _, err := svc.Replace(teamFolder.ID, nil, owner); err != nil {
...
func TestServiceResolveForFileChain(t *testing.T) {
	teamID, owner, member := uuid.New(), uuid.New(), uuid.New()
	root := files.File{ID: uuid.New(), Name: "root", OwnerID: owner, Type: "folder", ScopeType: "team", TeamID: &teamID, IsRoot: true}
	sub := files.File{ID: uuid.New(), Name: "sub", OwnerID: owner, Type: "folder", ScopeType: "team", TeamID: &teamID, ParentID: &root.ID}
	doc := files.File{ID: uuid.New(), Name: "doc.md", OwnerID: owner, Type: "file", ScopeType: "team", TeamID: &teamID, ParentID: &sub.ID}
	repo := NewMemoryRepo()
	repo.PutFolder(root)
	repo.PutFolder(sub)
	repo.PutFolder(doc)
	...
	// 根节点全队 deny write；子目录对 member allow write → 文件求值取近端 allow。
	if err := repo.Replace(root.ID, []Entry{entry(SubjectTeam, teamID, EffectDeny, "write")}); err != nil {
		t.Fatal(err)
	}
	...
	allowed, matched, err := svc.ResolveForFile(doc.ID, teamID, member, "write")
	if err != nil {
		t.Fatal(err)
	}
	if !allowed || !matched {
	...
	allowed, matched, err = svc.ResolveForFile(sub.ID, teamID, member, "read")
	if err != nil {
		t.Fatal(err)
	}
	if allowed || matched {
	...
	// owner 不在 allow 名单：近端 user 条目不匹配、远端 team deny 命中 → 拒绝。
	allowed, matched, err = svc.ResolveForFile(doc.ID, teamID, owner, "write")
	if err != nil {
		t.Fatal(err)
	}
	if allowed || !matched {
	...
	allowed, matched, err = svc.ResolveForFile(uuid.New(), teamID, member, "write")

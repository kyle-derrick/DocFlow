...
| 工具 | 一句话说明 | scope |
| --- | --- | --- |
| `df_list_files` | 列出目录内容（空间文件，limit/offset 分页） | files:read |
| `df_get_file` | 文件/目录元数据 + 当前版本（大小/sha256/MIME/状态） | files:read |
| `df_create_folder` | 创建子目录（继承所在空间作用域） | files:write |
...
| `df_list_trash` | 列出回收站条目（空间范围） | files:read |
...
| `df_list_spaces` | 列出可见空间（定位 space_id） | files:read |
| `df_resolve_path` | 按路径定位文件返回 file_id（如 `docs/报告.md`） | files:read |
| `df_download_url` | 返回下载/预览 API 路径（大文件引导） | files:read |

说明：

- `space_id` 参数缺省为用户默认空间；`df_list_spaces` 可列出全部可见空间（直接成员或经用户组）
- 跨空间操作（读/写/建目录/回收站）显式传 `space_id`，要求在册成员权限
...
- **权限模型零旁路**：所有工具直接调用现有服务层（`authorizeFile*`、空间角色 CanRead/CanWrite/CanDelete、路径级 ACL、PAT scope），MCP 层不做任何额外放行
...
## 作为 MCP 客户端（消费外部 MCP 服务）

除上文对外暴露工具外，DocFlow 的对话也能调用**外部** MCP 服务器：

- **管理员配置**：平台管理 → AI 设置 → MCP 工具服务，添加外部服务——URL 为 Streamable HTTP 端点，可附鉴权头（最多 8 个），保存前支持测试连接
- **用户使用**：在对话（AI 助手 / 编辑页 / 创作空间）中开启「MCP 工具」开关后，模型即可调用这些服务暴露的工具
- **协议与执行**：支持 initialize / tools.list / tools.call；单次对话工具调用最多 5 轮，SSE 流式展示每次工具调用过程
